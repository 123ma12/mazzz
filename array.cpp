...
Array::Array(int row ,int col,int boomNum )
{
    //动态创建二维数组，用于保存块信息
    p = new int *[row];
    for(int i = 0;i<row;i++)
...
int Array::getboomNum()
{
    return boomNum;
}
int Array::gettimeNum()
{
    return timeNum;
}
int Array::getredFlagNum()
{
    return redFlagNum;
}
void Array::minusredFlagNum()
{
    redFlagNum = redFlagNum -1;
}
void Array::addtimeNum()
{
    timeNum = timeNum + 1;
}
void Array::addredFlagNum()
{
     redFlagNum = redFlagNum + 1;
}
